...
mod tests {
    ...
    #[test]
    fn config_declares_udp_resource_and_no_acknowledgements() {
        let config = SnmpTrapConfig {
            address: SocketListenAddr::SocketAddr("127.0.0.1:1162".parse().unwrap()),
        ...
        assert_eq!(
            config.resources(),
            vec![Resource::udp(
                "127.0.0.1:1162".parse::<std::net::SocketAddr>().unwrap()
            )]
        );
    ...
    async fn test_events_match_schema_definition_in_both_namespaces() {
        for log_namespace in [LogNamespace::Legacy, LogNamespace::Vector] {
            ...
            for message in [
                ...
                let log = event.as_log();
                assert!(log.get(event_path!("trap_oid")).is_some());
                match log_namespace {
                    LogNamespace::Legacy => {
                        assert_eq!(log["host"], "127.0.0.1".into());
                        assert_eq!(log["source_type"], "snmp_trap".into());
                    ...
                    LogNamespace::Vector => {
                        assert!(log.get(event_path!("host")).is_none());
                        assert_eq!(
                            log.metadata().value().get(path!("snmp_trap", "host")),
                            Some(&"127.0.0.1".into())
                        );
                        assert_eq!(
                            log.metadata().value().get(path!("vector", "source_type")),
                            Some(&"snmp_trap".into())
                        );
                    }
                }
    ...
    fn v1_trap() -> Vec<u8> {
        let mut pdu = Vec::new();
        pdu.extend(oid_value(&[1, 3, 6, 1, 4, 1, 8072, 3, 2, 10]));
        pdu.extend(encode_tlv(0x40, &[192, 168, 1, 100]));
        pdu.extend(encode_integer(6));
        pdu.extend(encode_integer(1));
        pdu.extend(timeticks(123_456));
        pdu.extend(encode_sequence(&[]));

        let mut message = Vec::new();
        message.extend(encode_integer(0));
